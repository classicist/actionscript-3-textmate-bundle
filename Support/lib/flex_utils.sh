...
require_file (){
	...
	if [[ !(-f "$2") ]]; then
		...
		cat <<-HTMOUT
				<h2>$1 404</h2>
				<p>The environment variable <code>\$$1</code> does not resolve to an actual file (looked in: $2).
				<br>
				<p>Command executed from:    		TM_PROJECT_DIR: #{$TM_PROJECT_DIR}</p>
				<p>Command executed in file:		TM_PROJECT_FILEPATH: #{$TM_PROJECT_FILEPATH}</p>
				<p>Command looked for flex sdk in: 	TM_FLEX_PATH: #{$TM_FLEX_PATH}</p>
				<p>Looked for Libraries in:			TM_AS3_LIB_PATH: #{$TM_AS3_LIB_PATH}</p>								
				<p>Project Variables set to:		TM_FLEX_FILE_SPECS: #{$TM_FLEX_FILE_SPECS}</p>
				<p>Project Variables set to:		TM_FLEX_OUTPUT: #{$TM_FLEX_OUTPUT}</p>
				<p>Project Variables set to:		TM_PROJECT_DIR: #{$TM_PROJECT_DIR}</p>
				<br>
				Configuration help can be found <a href="tm-file://$TM_BUNDLE_SUPPORT/html/help.html#conf">here.</a></p>
		HTMOUT
